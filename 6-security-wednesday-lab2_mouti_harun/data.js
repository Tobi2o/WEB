...
await User.create({ username: 'elon.musk', displayName: 'Elon Musk', password: bcrypt.hashSync('imdabo$$', salt1)});
var salt2 = bcrypt.genSaltSync(saltRounds);
await User.create({ username: 'donald.trump', displayName: 'Donald Trump', password: bcrypt.hashSync('1234', salt2) });
var salt3 = bcrypt.genSaltSync(saltRounds);
await User.create({ username: 'jane.doe', displayName: 'Jane Doe', password: bcrypt.hashSync('p@ssw0rd', salt3) });
