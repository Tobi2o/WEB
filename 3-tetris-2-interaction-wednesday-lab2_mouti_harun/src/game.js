...
export class Game extends Map {
  ...
  moveShape(id, col) {
    ...
    if (this.map.testShape(shape, shape.row, col, shape.rotation)) {
      shape.col = col;
    }
  }
  ...
  rotateShape(id, rotation) {
    ...
    if (rotation === "left") {
      newRotation = shapeTypes[shape.shapeType].length - 1;
  ...
  onMessage(playerId, message) {
    ...
    if (message instanceof MoveMessage) {
      this.moveShape(playerId, message.data);
    }
    if (message instanceof RotateMessage) {
      this.rotateShape(playerId, message.data);
    }
  }
}
