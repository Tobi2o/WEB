...
export class Shape {
    ...
    static getRandomShapeType() {
            return Math.trunc(Math.random() * shapeTypes.length);
    }
    ...
    const shapeType = shapeTypes[this.shapeType];
    const rotationIndex = rotation % shapeType.length;
    return shapeType[rotationIndex];
}
}
